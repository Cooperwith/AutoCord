...
def random_sleep(duration):
    """Sleep for `duration` + a random 1–8 seconds."""
    sleep_duration = duration + random.randint(1, 8)
...
def send_message(conn, channel_id, message_data, header_data):
    try:
        ...
        if 199 < resp.status < 300:
            print(f"{get_timestamp()} Message sent!")
    except Exception as e:
...
def chunk_text(text, size=2000):
    """Split text into chunks of <=2000 characters (Discord limit)."""
    ...
    while True:
        try:
            with open(MESSAGES_FILE, "r", encoding="utf-8") as file:
                full_message = file.read().strip()
        except FileNotFoundError:
            print(f"{get_timestamp()} Messages file not found.")
            return

        # Split into chunks if longer than Discord limit
        messages = chunk_text(full_message, 2000)
        ...
        for message in messages:
            ...
            random_sleep(delay_between_messages)
